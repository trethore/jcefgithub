...
relocatedFatJarFile="$artifactId-$version-all-relocated.jar"
...
deployCmd=(mvn -q --no-transfer-progress deploy:deploy-file
  ...
  -Dpackaging=jar "-Dfile=$jarFile" -DrepositoryId=github "-Durl=$GITHUB_PACKAGES_URL")

# Add POM if it exists
if [ -f "$pomFile" ]; then
  deployCmd+=("-DpomFile=$pomFile")
else
  deployCmd+=(-DgeneratePom=true)
fi
...
# Add fat jar with classifier 'all-relocated' if it exists
if [ -f "$relocatedFatJarFile" ]; then
  deployCmd+=("-Dfiles=$relocatedFatJarFile" -Dclassifiers=all-relocated -Dtypes=jar)
fi
