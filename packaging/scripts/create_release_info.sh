...
# Set build info
. packaging/scripts/set_build_info.sh "$1" "$4"
...
(
  ...
  echo "##### Windows ARM64"
  echo "\`\`\`"
  echo "implementation 'io.github.trethore:jcef-natives-windows-arm64:jcef-$jcef_commit+cef-$cef_version'"
  echo "\`\`\`"
  echo "</details>"
  echo ""
...
# Add build_meta.json
curl --fail --show-error --silent --location -o build_meta.json "$1"
